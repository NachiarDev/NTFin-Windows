...
namespace Estimate
{
    public partial class ZipTypeList : Form
    {
        public ZipTypeList()
        {
            InitializeComponent();
        ...
        private void ZipTypeList_Load(object sender, EventArgs e)
        {
        ...
        private void LoadZipgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_ZipTypeGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM ZipType WHERE ID = @ID"; // Change table name properly
        ...
        {
            ZipType newForm = new ZipType(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1")
                {
                    ...
                    ZipType newForm = new ZipType();

                    newForm.Controls["textBox2"].Text = selectedRow.Cells["ID"].Value?.ToString();
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["CharType"].Value?.ToString();

...
namespace Estimate
{
    public partial class LocationList : Form
    {
        ...
        private void LocationList_Load(object sender, EventArgs e)
        {
            ...
            {
                column.FillWeight = 1; // Equal column size
            }
            LoadLocationgrid();
        ...
        private void LoadLocationgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_LocationGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Tbl_Location WHERE ID = @ID"; // Change table name properly
        ...
        {
            Location newForm = new Location(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1") 
                {
                    ...
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["LocationName"].Value?.ToString();
                    ...
                    {
                        int orgidId = Convert.ToInt32(selectedRow.Cells["OrgID"].Value ?? 0);
                        ...
                        if (newForm.Controls["comboBox1"] is ComboBox countryCombo)
                        {
                            using (SqlConnection con = new SqlConnection("Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;"))
                            {
                                ...
                                SqlDataAdapter da = new SqlDataAdapter("SELECT ID, Name FROM Branch WHERE Active = 1", con);
                                ...
                                countryCombo.SelectedValue = orgidId;

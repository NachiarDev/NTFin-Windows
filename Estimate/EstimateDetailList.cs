...
namespace Estimate
{
    public partial class EstimateDetailList : Form
    {
        ...
        private void EstimateDetailList_Load(object sender, EventArgs e)
        {
            MakeColumnsReadOnlyExceptButtons();
            LoadQuotationgrid();
            dataGridView1.Columns["EsdetailID"].Visible = false;
        ...
        private void LoadQuotationgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_InvoiceDetailGetGrid", conn))
                    {
        ...
        private void DeleteRowData(int rowIndex)
        {
            ...
            var id = row.Cells["EsdetailID"].Value?.ToString();
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Estimate_Details WHERE EsdetailID = @EsdetailID"; // Change table name properly

                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@EsdetailID", id);
                        cmd.ExecuteNonQuery();
                    }
                }
            }
        ...
        {
            Form1 newForm = new Form1(); // Create an instance of Form2
            newForm.ShowDialog();

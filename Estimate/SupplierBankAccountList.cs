...
namespace Estimate
{
    public partial class SupplierBankAccountList : Form
    {
        public SupplierBankAccountList()
        {
            InitializeComponent();
        }

        private void SupplierBankAccountList_Load(object sender, EventArgs e)
        {
            MakeColumnsReadOnlyExceptButtons();
            LoadSupplierBankAccgrid();

            dataGridView1.Columns["ID"].Visible = false;
            dataGridView1.Columns["SupplierID"].Visible = false;
            dataGridView1.Columns["SupliBankName"].HeaderText = "Bank Name";
            dataGridView1.Columns["SupliBankBranch"].HeaderText = "Bank Branch";
            dataGridView1.Columns["SupliBankAccNo"].HeaderText = "Account Number";
            AddButtonColumns();
        ...
        private void LoadSupplierBankAccgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_SupplierBankAccountGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Supplier_BankAccount WHERE ID = @ID"; // Change table name properly
        ...
        {
            SupplierBankAccount newForm = new SupplierBankAccount(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1") // View button clicked
                {
                    ...
                    SupplierBankAccount newForm = new SupplierBankAccount();

                    newForm.Controls["textBox7"].Text = selectedRow.Cells["ID"].Value?.ToString();
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["SupliBankName"].Value?.ToString();
                    newForm.Controls["textBox2"].Text = selectedRow.Cells["SupliBankBranch"].Value?.ToString();
                    newForm.Controls["textBox3"].Text = selectedRow.Cells["SupliBankAccNo"].Value?.ToString();
                    newForm.Controls["textBox4"].Text = selectedRow.Cells["SupliBankIFSC"].Value?.ToString();
                    newForm.Controls["textBox5"].Text = selectedRow.Cells["SwiftCode"].Value?.ToString();
                    newForm.Controls["textBox6"].Text = selectedRow.Cells["SwiftBranch"].Value?.ToString();
                    newForm.Controls["richTextBox1"].Text = selectedRow.Cells["SwiftDetails"].Value?.ToString();
                    newForm.ShowDialog();
                }

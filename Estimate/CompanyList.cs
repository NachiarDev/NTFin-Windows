    ...
    public partial class CompanyList : Form
    {
        ...
        private void LoadCountrygrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_CompanyGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Company WHERE ID = @ID"; // Change table name properly
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1")
                {
                    ...
                    Company newForm = new Company();

                    newForm.Controls["textBox4"].Text = selectedRow.Cells["ID"].Value?.ToString();
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["Name"].Value?.ToString();
                    newForm.Controls["textBox3"].Text = selectedRow.Cells["GSTNumber"].Value?.ToString();
                    newForm.Controls["maskedTextBox1"].Text = selectedRow.Cells["Contact"].Value?.ToString();
        ...
        {
            Company newForm = new Company(); // Create an instance of Form2
            newForm.ShowDialog();

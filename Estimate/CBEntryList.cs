...
namespace Estimate
{
    public partial class CBEntryList : Form
    {
        public CBEntryList()
        {
            InitializeComponent();
        ...
        private void CBEntryList_Load(object sender, EventArgs e)
        {
            MakeColumnsReadOnlyExceptButtons();
            LoadExpensesList();
            dataGridView1.Columns["ID"].Visible = false;
            dataGridView1.Columns["TransactionType"].Visible = false;
        ...
        private void LoadExpensesList()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_CBEntryGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM CBEntry WHERE ID = @ID"; // Change table name properly
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1") // View button clicked
                {
                    ...
                    CBEntry newForm = new CBEntry();

                    newForm.Controls["textBox4"].Text = selectedRow.Cells["ID"].Value?.ToString();
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["TotalAmount"].Value?.ToString();
                    newForm.Controls["textBox2"].Text = selectedRow.Cells["Amount"].Value?.ToString();

                    newForm.Controls["textBox3"].Text = selectedRow.Cells["Balance"].Value?.ToString();
                    ...
                    newForm.Shown += (s, args) =>
                    {
                        ...
                        int InvoiceId = Convert.ToInt32(selectedRow.Cells["InvoiceNo"].Value ?? 0);
                        if (newForm.Controls["comboBox1"] is ComboBox InvoiceCombo)
                        {
                            using (SqlConnection con = new SqlConnection("Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;"))
                            {
                                ...
                                SqlDataAdapter paymethodDa = new SqlDataAdapter("SELECT ID, CAST(RefNo AS VARCHAR) + ' - ' + CustomerCode AS DisplayText \r\n            FROM Estimate", con);

                               
                                DataTable dtpayemntmethod = new DataTable();
                                paymethodDa.Fill(dtpayemntmethod);
                                InvoiceCombo.DisplayMember = "DisplayText";
                                InvoiceCombo.ValueMember = "ID";
                                InvoiceCombo.DataSource = dtpayemntmethod;
                                InvoiceCombo.SelectedValue = InvoiceId;
                            }
                        }                        
        ...
        {
            CBEntry newForm = new CBEntry(); // Create an instance of Form2
            newForm.ShowDialog();
        }
    }
}

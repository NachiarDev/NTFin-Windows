...
namespace Estimate
{
    public partial class BillAddressList : Form
    {
        public BillAddressList()
        {
            InitializeComponent();
        }
        private void BillAddressList_Load(object sender, EventArgs e)
        {
        ...
        private void LoadCountrygrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_BillAddressGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM BillAddress WHERE ID = @ID"; // Change table name properly
        ...
        {
            BillAddress newForm = new BillAddress(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1")
                {
                    ...
                    BillAddress newForm = new BillAddress();

                    newForm.Controls["textBox1"].Text = selectedRow.Cells["ID"].Value?.ToString();

...
namespace Estimate
{
    public partial class PurchaseReturnDetailList : Form
    {
        public PurchaseReturnDetailList()
        {
            InitializeComponent();
        }


        private void PurchaseReturnDetailList_Load(object sender, EventArgs e)
        {
            MakeColumnsReadOnlyExceptButtons();
            LoadPurchasereturnlistgrid();
            dataGridView1.Columns["PRdetailsID"].Visible = false;
           
            dataGridView1.Columns["Qty"].HeaderText = "Quantuty";
        ...
        private void LoadPurchasereturnlistgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_PurchaseReturnDetailGetGrid", conn))
                    {
        ...
        private void DeleteRowData(int rowIndex)
        {
            ...
            var id = row.Cells["PRdetailsID"].Value?.ToString();
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM PurchaseReturn_Details WHERE PRdetailsID = @PRdetailsID"; // Change table name properly

                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@PRdetailsID", id);
                        cmd.ExecuteNonQuery();
                    }
                }
            }
        ...
        {
            PurchaseReturn newForm = new PurchaseReturn(); // Create an instance of Form2
            newForm.ShowDialog();
        }

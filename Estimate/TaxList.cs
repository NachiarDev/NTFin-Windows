...
namespace Estimate
{
    public partial class TaxList : Form
    {
        ...
        private void TaxList_Load(object sender, EventArgs e)
        {
        ...
        private void Loadtaxgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_TaxGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Tax WHERE ID = @ID"; // Change table name properly
        ...
        {
            TaxForm newForm = new TaxForm(); // Create an instance of Form2
            newForm.ShowDialog();
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1")
                {
                    ...
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["gsttype"].Value?.ToString();
        ...
        {
            // Example: reload your data into a DataGridView
            string connStr = "Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;";
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                conn.Open();
                SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM Tax", conn);

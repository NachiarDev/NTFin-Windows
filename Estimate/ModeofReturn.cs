...
namespace Estimate
{
    public partial class ModeofReturn : Form
    {
        public ModeofReturn()
        {
            InitializeComponent();
        ...
        private void ModeofReturn_Load(object sender, EventArgs e)
        {
            ...
            var fields = new Dictionary<System.Windows.Forms.Control, string>
    {
         ...
         { textBox1, "Return Type is required" },
        ...
        private async void InsertData()
        {
...

            {
                ...
                using (SqlConnection conn = new SqlConnection("Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;"))
                {
                    ...
                    {
                        int returnId = int.Parse(textBox2.Text);

                        string updateQuery = @"
                    UPDATE Mode_of_Return 
                    SET ReturnType = @ReturnType
                    WHERE ID = @ID";

                        using (SqlCommand cmd = new SqlCommand(updateQuery, conn))
                        {
                            cmd.Parameters.AddWithValue("@ReturnType", textBox1.Text);
                    ...
                    {
                        string checkQuery = "SELECT COUNT(*) FROM Mode_of_Return WHERE ReturnType = @ReturnType";

                        using (SqlCommand checkCmd = new SqlCommand(checkQuery, conn))
                        {
                            checkCmd.Parameters.AddWithValue("@ReturnType", textBox1.Text);
                        ...
                        string insertQuery = @"
                    INSERT INTO Mode_of_Return (ReturnType) 
                    VALUES (@ReturnType);

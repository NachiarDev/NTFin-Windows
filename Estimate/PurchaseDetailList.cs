...
namespace Estimate
{
    public partial class PurchaseDetailList : Form
    {
        ...
        private void PurchaseDetailList_Load(object sender, EventArgs e)
        {
            ...
            LoadPurchasegrid();
            dataGridView1.Columns["PurDetailsID"].Visible = false;
        ...
        private void LoadPurchasegrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_PurchaseDetailGetGrid", conn))
                    {
        ...
        private void DeleteRowData(int rowIndex)
        {
            ...
            var id = row.Cells["PurDetailsID"].Value?.ToString();
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Purchase_Detail WHERE PurDetailsID = @PurDetailsID"; // Change table name properly

                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@PurDetailsID", id);
                        cmd.ExecuteNonQuery();
                    }
                }
            }
        ...
        {
            Purchase newForm = new Purchase(); // Create an instance of Form2
            newForm.ShowDialog();
        }

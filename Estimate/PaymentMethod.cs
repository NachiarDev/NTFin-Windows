...
namespace Estimate
{
            ...
            var fields = new Dictionary<System.Windows.Forms.Control, string>
    {
         ...
         { textBox1, "Method Name is required" },
        ...
        private async void InsertData()
        {
...

            {
                ...
                using (SqlConnection conn = new SqlConnection("Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;"))
                {
                    ...
                    {
                        int methodId = int.Parse(textBox2.Text);
                    ...
                    WHERE ID = @ID";
                        ...
                        {
                            cmd.Parameters.AddWithValue("@MethodName", textBox1.Text);
                    ...
                    {
                        string checkQuery = "SELECT COUNT(*) FROM PaymentMethod WHERE MethodName = @MethodName";

                        using (SqlCommand checkCmd = new SqlCommand(checkQuery, conn))
                        {
                            checkCmd.Parameters.AddWithValue("@MethodName", textBox1.Text);

                            int count = (int)await checkCmd.ExecuteScalarAsync();

                            if (count > 0)
                            {
                                MessageBox.Show("This PaymentMethod Type Existing. Please enter different details.", "Duplicate Entry", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                return;
                            }
                        }

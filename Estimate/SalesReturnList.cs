...
namespace Estimate
{
    public partial class SalesReturnList : Form
    {
        ...
        private void SalesReturnList_Load(object sender, EventArgs e)
        {
            ...
            dataGridView1.Columns["ModeofReturn"].Visible = false;
            dataGridView1.Columns["StateSupply"].Visible = false;
            dataGridView1.Columns["StateName"].HeaderText = "State Supply";
            dataGridView1.Columns["RefNo"].HeaderText = "SalesReturn Number";
            AddButtonColumns();
        ...
        private void LoadSalesReturngrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_SalesReturnGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM SalesReturn WHERE ID = @ID"; // Change table name properly

...
namespace Estimate
{
    public partial class ZipLengthList : Form
    {
        public ZipLengthList()
        {
            InitializeComponent();
        }

        private void ZipLengthList_Load(object sender, EventArgs e)
        {
        ...
        private void LoadZipgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_ZipLengthGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM ZipLength WHERE ID = @ID"; // Change table name properly
        ...
        {
            ZipLength newForm = new ZipLength(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1")
                {
                    ...
                    ZipLength newForm = new ZipLength();

                    newForm.Controls["textBox2"].Text = selectedRow.Cells["ID"].Value?.ToString();
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["CharSize"].Value?.ToString();

        ...
        private async void InsertData()
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection("Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;"))
                {
                    ...
                    string SizeName = textBox1.Text;

                    if (!string.IsNullOrWhiteSpace(textBox2.Text)) // UPDATE logic
                    {
                        int sizeId = int.Parse(textBox2.Text);

                        string updateQuery = @"
                    UPDATE Size 
                    SET SizeName = @SizeName, Active = @Active
                    WHERE ID = @ID";
                    ...
                    {
                        string checkQuery = "SELECT COUNT(*) FROM Size WHERE  SizeName = @SizeName";

                        using (SqlCommand checkCmd = new SqlCommand(checkQuery, conn))
                        {
                            checkCmd.Parameters.AddWithValue("@SizeName", SizeName);
                        ...
                        string insertQuery = @"
                    INSERT INTO Size (SizeName, Active) 
                    VALUES (@SizeName, @Active);

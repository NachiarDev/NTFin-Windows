...
namespace Estimate
{
    public partial class SalesReturnDatailList : Form
    {
        public SalesReturnDatailList()
        {
            InitializeComponent();
        }

        private void SalesReturnDatailList_Load(object sender, EventArgs e)
        {
            MakeColumnsReadOnlyExceptButtons();
            LoadSalesreturnlistgrid();
            dataGridView1.Columns["SRdetailID"].Visible = false;
            dataGridView1.Columns["UnitType"].Visible = false;
            dataGridView1.Columns["TaxType"].Visible = false;
            dataGridView1.Columns["SRReturnID"].HeaderText = "Invoice Number";
        ...
        private void LoadSalesreturnlistgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_SalesReturnDetailsGetGrid", conn))
                    {
        ...
        {
            SalesReturn newForm = new SalesReturn(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void DeleteRowData(int rowIndex)
        {
            ...
            var id = row.Cells["SRdetailID"].Value?.ToString();
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM SalesReturn_Details WHERE SRdetailID = @SRdetailID"; // Change table name properly

                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@SRdetailID", id);
                        cmd.ExecuteNonQuery();
                    }
                }
            }

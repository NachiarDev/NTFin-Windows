...
namespace Estimate
{
...

            {
                ...
                using (SqlConnection conn = new SqlConnection("Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;"))
                {
                    ...
                    WHERE ID = @ID";
                        ...
                        {
                            cmd.Parameters.AddWithValue("@StatusName", textBox1.Text);
                    ...
                    else // INSERT logic
                    {
                        ...
                        {
                            checkCmd.Parameters.AddWithValue("@StatusName", textBox1.Text);

                            int count = (int)await checkCmd.ExecuteScalarAsync();

                            if (count > 0)
                            {
                                MessageBox.Show("This Status Name Existing. Please enter different details.", "Duplicate Entry", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                return;
                            }
                        }
            ...
            var fields = new Dictionary<System.Windows.Forms.Control, string>
    {
         ...
         { textBox1, "Status Name is required" },

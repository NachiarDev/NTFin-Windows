...
namespace Estimate
{
    public partial class CustomerBankAccountList : Form
    {
        public CustomerBankAccountList()
        {
            InitializeComponent();
        }

        private void CustomerBankAccountList_Load(object sender, EventArgs e)
        {
            MakeColumnsReadOnlyExceptButtons();
            LoadCustomerBankAccgrid();
            dataGridView1.Columns["ID"].Visible = false;
            dataGridView1.Columns["CustomerID"].Visible = false;
          
            dataGridView1.Columns["CustBankName"].HeaderText = "Bank Name";
            dataGridView1.Columns["CustBankBranch"].HeaderText = "Bank Branch";
            dataGridView1.Columns["CustBankAccNo"].HeaderText = "Account Number";
        ...
        private void LoadCustomerBankAccgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_CustomerBankAccountGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Customer_BankAccount WHERE ID = @ID"; // Change table name properly
        ...
        {
            CustomerBanckAccount newForm = new CustomerBanckAccount(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1") // View button clicked
                {
                    ...
                    CustomerBanckAccount newForm = new CustomerBanckAccount();

                    // Set values into Organisation form controls
                    newForm.Controls["textBox7"].Text = selectedRow.Cells["ID"].Value?.ToString();
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["CustBankName"].Value?.ToString();
                    newForm.Controls["textBox2"].Text = selectedRow.Cells["CustBankBranch"].Value?.ToString();
                    newForm.Controls["textBox3"].Text = selectedRow.Cells["CustBankAccNo"].Value?.ToString();
                    newForm.Controls["textBox4"].Text = selectedRow.Cells["CustBankIFSC"].Value?.ToString();

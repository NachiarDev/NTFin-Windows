...
WHERE ID = @ID";
                    ...
                    else
                    {
                        // Insert Mode
                        string insertQuery = @"
INSERT INTO Supplier_Address (SupplierID, Address, AddressTypeID, ZipCode, CountryID, StateID, BnchID, LocID, CreatedBy, CreatedDate) 
VALUES (@SupplierID, @Address, @AddressTypeID, @ZipCode, @CountryID, @StateID, @BnchID, @LocID, @CreatedBy, GETDATE());

SELECT SCOPE_IDENTITY();";

                        using (SqlCommand cmd = new SqlCommand(insertQuery, conn))
                        {
                            cmd.Parameters.AddWithValue("@SupplierID", 8); // Set dynamically if needed
                            cmd.Parameters.AddWithValue("@Address", richTextBox1.Text);
                            cmd.Parameters.AddWithValue("@AddressTypeID", comboBox3.SelectedValue ?? DBNull.Value);
                            cmd.Parameters.AddWithValue("@ZipCode", textBox1.Text);
                            cmd.Parameters.AddWithValue("@CountryID", comboBox1.SelectedValue ?? DBNull.Value);
    ...
    {
                {richTextBox1,"SupplierAdddress is Important "},

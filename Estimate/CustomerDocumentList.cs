...
namespace Estimate
{
    public partial class CustomerDocumentList : Form
    {
        public CustomerDocumentList()
        {
            InitializeComponent();
        }

        private void CustomerDocumentList_Load(object sender, EventArgs e)
        {
            LoadCustomerdocumentgrid();
        ...
        private void LoadCustomerdocumentgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_CustomerDocumentGetGrid", conn))
                    {
        ...
        {
            CustomerDocument newForm = new CustomerDocument(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM CusDoc WHERE ID = @ID"; // Change table name properly

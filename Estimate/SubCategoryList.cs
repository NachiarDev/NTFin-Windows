    ...
    public partial class SubCategoryList : Form
    {
        ...
        private void LoadCategorygrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_SubCategoryGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM SubCategory WHERE ID = @ID"; // Change table name properly
        ...
        {
            SubCategory newForm = new SubCategory(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1")
                {
                    ...
                    SubCategory newForm = new SubCategory();

                    newForm.Controls["textBox2"].Text = selectedRow.Cells["ID"].Value?.ToString();
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["SubCategoryName"].Value?.ToString();
                    // Set Active Status (radio buttons)
                    

                    newForm.Shown += (s, args) =>
                    {
                        int categoryId = Convert.ToInt32(selectedRow.Cells["CategoryID"].Value ?? 0);
                        ...
                        if (newForm.Controls["comboBox1"] is ComboBox countryCombo)
                        {
                            using (SqlConnection con = new SqlConnection("Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;"))
                            {
                                ...
                                countryCombo.DataSource = dtCategories;
                                countryCombo.SelectedValue = categoryId;

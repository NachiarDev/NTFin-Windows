...
namespace Estimate
{
    public partial class CustomerAddressList : Form
    {
        public CustomerAddressList()
        {
            InitializeComponent();
        }
        private void CustomerAddressList_Load(object sender, EventArgs e)
        {
            MakeColumnsReadOnlyExceptButtons();
            LoadCustomerAddressgrid();
        ...
        private void LoadCustomerAddressgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_CustomerAddressGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Customer_Address WHERE ID = @ID"; // Change table name properly
        ...
        {
            CustomerAddress newForm = new CustomerAddress(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1") // View button clicked
                {
                    ...
                    CustomerAddress newForm = new CustomerAddress();

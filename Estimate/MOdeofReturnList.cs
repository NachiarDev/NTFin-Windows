...
namespace Estimate
{
    public partial class MOdeofReturnList : Form
    {
        public MOdeofReturnList()
        {
            InitializeComponent();
        }

        private void MOdeofReturnList_Load(object sender, EventArgs e)
        {
        ...
        private void LoadUnitgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_ModeofReturnGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Mode_of_Return WHERE ID = @ID"; // Change table name properly
        ...
        {
            ModeofReturn newForm = new ModeofReturn(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1")
                {
                    ...
                    ModeofReturn newForm = new ModeofReturn();

                    newForm.Controls["textBox2"].Text = selectedRow.Cells["ID"].Value?.ToString();
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["ReturnType"].Value?.ToString();

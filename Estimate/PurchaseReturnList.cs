...
namespace Estimate
{
    public partial class PurchaseReturnList : Form
    {
        ...
        private void PurchaseReturnList_Load(object sender, EventArgs e)
        {
            ...
            dataGridView1.Columns["StatePurchase"].Visible = false;
        ...
        private void LoadPurchasereturnlistgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_PurchaseReturnGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM PurchaseReturn WHERE ID = @ID"; // Change table name properly

...
namespace Estimate
{
    public partial class ZipLength : Form
    {
        public ZipLength()
        {
            InitializeComponent();
        ...
        private void ZipLength_Load(object sender, EventArgs e)
        {
...

            {
                ...
                using (SqlConnection conn = new SqlConnection("Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;"))
                {
                    ...
                    WHERE ID = @ID";
                        ...
                        {
                            cmd.Parameters.AddWithValue("@CharSize", textBox1.Text);
                    ...
                    {
                        string checkQuery = "SELECT COUNT(*) FROM ZipLength WHERE CharSize = @CharSize";

                        using (SqlCommand checkCmd = new SqlCommand(checkQuery, conn))
                        {
                            checkCmd.Parameters.AddWithValue("@CharSize", textBox1.Text);

                            int count = (int)await checkCmd.ExecuteScalarAsync();

                            if (count > 0)
                            {
                                MessageBox.Show("This Character Length Existing. Please enter different details.", "Duplicate Entry", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                return;
                            }
                        }

                        string insertQuery = @"
                    INSERT INTO ZipLength (CharSize) 
                    VALUES (@CharSize);
            ...
            var fields = new Dictionary<System.Windows.Forms.Control, string>
    {
         ...
         { textBox1, "Character Length is required" },

...
namespace Estimate
{
    public partial class SupplierDocumentList : Form
    {
        public SupplierDocumentList()
        {
            InitializeComponent();
        }

        private void SupplierDocumentList_Load(object sender, EventArgs e)
        {
            LoadSupplierdocumentgrid();
        ...
        private void LoadSupplierdocumentgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_SupplierDocumentGetGrid", conn))
                    {
        ...
        {
            SupplierDocument newForm = new SupplierDocument(); // Create an instance of Form2
            newForm.ShowDialog();
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM SupDoc WHERE ID = @ID"; // Change table name properly
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1")
                {
                    ...
                    CustomerDocument newForm = new CustomerDocument();

...
namespace Estimate
{
    public partial class QuotationDetailList : Form
    {
        ...
        private void QuotationDetailList_Load(object sender, EventArgs e)
        {
            MakeColumnsReadOnlyExceptButtons();
            LoadQuotationgrid();
            dataGridView1.Columns["QuotdetailID"].Visible = false;
            dataGridView1.Columns["UnitType"].Visible = false;
            dataGridView1.Columns["TaxType"].Visible = false;
            dataGridView1.Columns["InvoNo"].HeaderText = "Invoice Number";
        ...
        private void LoadQuotationgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_QuotationDetailGetGrid", conn))
                    {
        ...
        private void DeleteRowData(int rowIndex)
        {
            ...
            var id = row.Cells["QuotdetailID"].Value?.ToString();
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Quotation_Details WHERE QuotdetailID = @QuotdetailID"; // Change table name properly

                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@QuotdetailID", id);
                        cmd.ExecuteNonQuery();
                    }
                }
            }

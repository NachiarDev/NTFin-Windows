...
namespace Estimate
{
    public partial class StockINList : Form
    {
        public StockINList()
        {
            InitializeComponent();
        ...
        private void StockINList_Load(object sender, EventArgs e)
        {
            ...
            dataGridView1.Columns["serialno"].HeaderText = "Serial Number";
            AddButtonColumns();
        ...
        private void LoadStockINgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_StockInGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM StockIN WHERE ID = @ID"; // Change table name properly
        ...
        {
            StockIN newForm = new StockIN(); // Create an instance of Form2
            newForm.ShowDialog();
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1") // View button clicked
                {
                    ...
                    StockIN newForm = new StockIN();

                    // Set values into Organisation form controls
                    newForm.Controls["textBox5"].Text = selectedRow.Cells["ID"].Value?.ToString();
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["batch"].Value?.ToString();
                    newForm.Controls["textBox2"].Text = selectedRow.Cells["serialno"].Value?.ToString();
                    newForm.Controls["textBox3"].Text = selectedRow.Cells["price"].Value?.ToString();
                    newForm.Controls["textBox4"].Text = selectedRow.Cells["TotalQty"].Value?.ToString();
                    newForm.Shown += (s, args) =>
                    {
                        string productId = selectedRow.Cells["ProductName"].Value ?.ToString();
                        ...
                        if (newForm.Controls["comboBox1"] is ComboBox countryCombo)
                        {
                            ...
                            using (SqlConnection con = new SqlConnection("Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;"))
                            {
                                ...
                                countryCombo.ValueMember = "ProductName";
                                countryCombo.DataSource = dtproducts;

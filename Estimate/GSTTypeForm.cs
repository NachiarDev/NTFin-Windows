...
namespace Estimate
{
    public partial class GSTTypeForm : Form
    {
        public GSTTypeForm()
        {
            InitializeComponent();
        ...
        private void GSTTypeForm_Load(object sender, EventArgs e)
        {
        ...
        private async void InsertData()
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection("Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;"))
                {
                    ...
                    string GSTType = textBox1.Text;

                    if (!string.IsNullOrWhiteSpace(textBox2.Text)) // UPDATE logic
                    {
                        int gsttypeId = int.Parse(textBox2.Text);

                        string updateQuery = @"
                    UPDATE Tbl_GSTType 
                    SET GSTType = @GSTType, Active = @Active
                    WHERE ID = @ID";
                        ...
                        using (SqlCommand cmd = new SqlCommand(updateQuery, conn))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@ID", gsttypeId);
                    ...
                    {
                        string checkQuery = "SELECT COUNT(*) FROM Tbl_GSTType WHERE  GSTType = @GSTType";

                        using (SqlCommand checkCmd = new SqlCommand(checkQuery, conn))
                        {
                            checkCmd.Parameters.AddWithValue("@GSTType", GSTType);
                        ...
                        string insertQuery = @"
                    INSERT INTO Tbl_GSTType (GSTType, Active) 
                    VALUES (@GSTType, @Active);

...
namespace Estimate
{
    public partial class CountryList : Form
    {
        ...
        private void CountryList_Load(object sender, EventArgs e)
        {
            ...
            dataGridView1.Columns["CountryNumber"].Visible = false;
        ...
        private void LoadCountrygrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_CountryGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Tbl_Country WHERE ID = @ID"; // Change table name properly
        ...
        {
            Country newForm = new Country(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1")
                {
                    ...
                    Country newForm = new Country();

                    newForm.Controls["textBox4"].Text = selectedRow.Cells["ID"].Value?.ToString();
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["CountryCode"].Value?.ToString();
                    newForm.Controls["textBox2"].Text = selectedRow.Cells["CountryNumber"].Value?.ToString();

                    newForm.Controls["textBox3"].Text = selectedRow.Cells["CountryName"].Value?.ToString();

...
namespace Estimate
{
    public partial class ZipType : Form
    {
        public ZipType()
        {
            InitializeComponent();
        ...
        private void ZipType_Load(object sender, EventArgs e)
        {
...

            {
                ...
                using (SqlConnection conn = new SqlConnection("Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;"))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(textBox2.Text)) // UPDATE logic
                    {
                        ...
                        string updateQuery = @"
                    UPDATE ZipType 
                    SET CharType = @CharType
                    WHERE ID = @ID";

                        using (SqlCommand cmd = new SqlCommand(updateQuery, conn))
                        {
                            cmd.Parameters.AddWithValue("@CharType", textBox1.Text);
                    ...
                    {
                        string checkQuery = "SELECT COUNT(*) FROM ZipType WHERE CharType = @CharType";

                        using (SqlCommand checkCmd = new SqlCommand(checkQuery, conn))
                        {
                            checkCmd.Parameters.AddWithValue("@CharType", textBox1.Text);

                            int count = (int)await checkCmd.ExecuteScalarAsync();

                            if (count > 0)
                            {
                                MessageBox.Show("This Character Type Existing. Please enter different details.", "Duplicate Entry", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                return;
                            }
                        }

                        string insertQuery = @"
                    INSERT INTO ZipType (CharType) 
                    VALUES (@CharType);
            ...
            var fields = new Dictionary<System.Windows.Forms.Control, string>
    {
         ...
         { textBox1, "Character Type is required" },

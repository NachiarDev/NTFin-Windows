...
namespace Estimate
{
    public partial class CountrySettingList : Form
    {
        public CountrySettingList()
        {
            InitializeComponent();
        }

        private void CountrySettingList_Load(object sender, EventArgs e)
        {
        ...
        private void LoadCountrygrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_Country_SettingsGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Country_Settings WHERE ID = @ID"; // Change table name properly
        ...
        {
            CountrySetting newForm = new CountrySetting(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1")
                {
                    ...
                    CountrySetting newForm = new CountrySetting();

                    newForm.Controls["textBox6"].Text = selectedRow.Cells["ID"].Value?.ToString();
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["CountryName"].Value?.ToString();
                    newForm.Controls["textBox2"].Text = selectedRow.Cells["CountryCode"].Value?.ToString();
                    newForm.Controls["textBox3"].Text = selectedRow.Cells["CurrencyName"].Value?.ToString();
                    newForm.Controls["textBox4"].Text = selectedRow.Cells["CurrencyCode"].Value?.ToString();
                    newForm.Controls["textBox5"].Text = selectedRow.Cells["CurrencySymbol"].Value?.ToString();

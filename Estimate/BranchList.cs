...
namespace Estimate
{
    public partial class BranchList : Form
    {
        ...
        private void BranchList_Load(object sender, EventArgs e)
        {
        ...
        private void LoadBranchgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_BranchGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Branch WHERE ID = @ID"; // Change table name properly
        ...
        {
            Branch newForm = new Branch(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1") // View button clicked
                {
                    ...
                    Branch newForm = new Branch();

                    newForm.Controls["textBox4"].Text = selectedRow.Cells["ID"].Value?.ToString();
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["Code"].Value?.ToString();
                    newForm.Controls["textBox2"].Text = selectedRow.Cells["Name"].Value?.ToString();
                    newForm.Controls["maskedTextBox1"].Text = selectedRow.Cells["ContactNo"].Value?.ToString();
                    newForm.Controls["maskedTextBox2"].Text = selectedRow.Cells["Mobile"].Value?.ToString();
                    newForm.Controls["textBox5"].Text = selectedRow.Cells["Email"].Value?.ToString();
                    newForm.Controls["richTextBox1"].Text = selectedRow.Cells["BranchAddress"].Value?.ToString();
                    newForm.Controls["textBox3"].Text = selectedRow.Cells["GSTNumber"].Value?.ToString();
                    newForm.Controls["textBox7"].Text = selectedRow.Cells["Zip"].Value?.ToString();
                    newForm.Controls["textBox6"].Text = selectedRow.Cells["City"].Value?.ToString();
                    ...
                    newForm.Shown += (s, args) =>
                    {
                        ...
                        int organisationId = Convert.ToInt32(selectedRow.Cells["OrgID"].Value ?? 0);
                        if (newForm.Controls["comboBox1"] is ComboBox orgCombo &&
                            newForm.Controls["comboBox2"] is ComboBox locationCombo)
                        {
                            using (SqlConnection con = new SqlConnection("Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;"))
                            {
                                ...
                                // Load organisations
                                SqlDataAdapter orgDa = new SqlDataAdapter("SELECT ID, Name FROM Organisation WHERE Active = 1", con);
                                DataTable dtOrgs = new DataTable();
                                orgDa.Fill(dtOrgs);
                                orgCombo.DisplayMember = "Name";
                                orgCombo.ValueMember = "ID";
                                orgCombo.DataSource = dtOrgs;
                                orgCombo.SelectedValue = organisationId;

                                // Load locations based on OrganisationID
                                SqlDataAdapter locDa = new SqlDataAdapter("SELECT ID, LocationName FROM Tbl_Location WHERE Active = 1 AND OrgID = @OrgID", con);
                                locDa.SelectCommand.Parameters.AddWithValue("@OrgID", organisationId);
                                DataTable dtLocs = new DataTable();
                                locDa.Fill(dtLocs);
                                locationCombo.DisplayMember = "LocationName";
                                locationCombo.ValueMember = "ID";
                                locationCombo.DataSource = dtLocs;
                                int locationId = Convert.ToInt32(selectedRow.Cells["Location"].Value ?? 0);
                                locationCombo.SelectedValue = locationId;
                            }
                        }
                        ...
                        if (newForm.Controls["comboBox3"] is ComboBox countryCombo &&
                            newForm.Controls["comboBox4"] is ComboBox stateCombo)
                        {
                            using (SqlConnection con = new SqlConnection("Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;"))
                            {
                                con.Open();

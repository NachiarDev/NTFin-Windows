...
namespace Estimate
{
    public partial class DeliveryChallanDetailsList : Form
    {
        public DeliveryChallanDetailsList()
        {
            InitializeComponent();
        }

        private void DeliveryChallanDetailsList_Load(object sender, EventArgs e)
        {
            MakeColumnsReadOnlyExceptButtons();
            LoadPurchasereturnlistgrid();
            dataGridView1.Columns["DSdetailID"].Visible = false;
            dataGridView1.Columns["ProductName"].Visible = false;
            dataGridView1.Columns["proname"].HeaderText = "ProductName";
        ...
        private void LoadPurchasereturnlistgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_Challan_DetailsGetGrid", conn))
                    {
        ...
        private void DeleteRowData(int rowIndex)
        {
            ...
            var id = row.Cells["DSdetailID"].Value?.ToString();
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Challan_Details WHERE DSdetailID = @DSdetailID"; // Change table name properly

                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@DSdetailID", id);
                        cmd.ExecuteNonQuery();
                    }
                }
            }

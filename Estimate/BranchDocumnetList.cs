...
namespace Estimate
{
    public partial class BranchDocumnetList : Form
    {
        public BranchDocumnetList()
        {
            InitializeComponent();
        }

        private void BranchDocumnetList_Load(object sender, EventArgs e)
        {
            Loadbranchdocumentgrid();
            MakeColumnsReadOnlyExceptButtons();
            dataGridView1.Columns["ID"].Visible = false;
            dataGridView1.Columns["BnchID"].Visible = false;

            dataGridView1.Columns["LocID"].Visible = false;
            dataGridView1.Columns["Name"].HeaderText = "Branch Name";

            dataGridView1.Columns["docFile"].HeaderText = "Image";
            AddButtonColumns();
        ...
        private void Loadbranchdocumentgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_BranchDocumentGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM BnchDoc WHERE ID = @ID"; // Change table name properly
        ...
        {
            BranchDocument newForm = new BranchDocument(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1")
                {
                    ...
                    BranchDocument newForm = new BranchDocument();

                    newForm.Controls["textBox1"].Text = selectedRow.Cells["FileName"].Value?.ToString();
                    newForm.Controls["textBox2"].Text = selectedRow.Cells["ID"].Value?.ToString();

                   
                    newForm.Shown += (s, args) =>
                    {
                        int bnchidId = Convert.ToInt32(selectedRow.Cells["BnchID"].Value ?? 0);
                      ...
                      using (SqlConnection con = new SqlConnection("Data Source=45.64.104.2;Initial Catalog=kalviaga_NTFin_Test;User ID=TestDBA;Password=N@tsys@2024;Encrypt=True;TrustServerCertificate=True;"))
                            {
                                ...
                                SqlDataAdapter da = new SqlDataAdapter("SELECT ID, Name FROM Branch", con);
                                DataTable dtOrgies = new DataTable();
                                da.Fill(dtOrgies);

                                countryCombo.DisplayMember = "Name";
                                countryCombo.ValueMember = "ID";
                                countryCombo.DataSource = dtOrgies;
                                countryCombo.SelectedValue = bnchidId;
                            }
                        }

                        if (newForm.Controls["pictureBox1"] is PictureBox picBox)
                        {
                            string imagePath = selectedRow.Cells["docFile"].Value?.ToString();

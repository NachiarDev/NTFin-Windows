...
namespace Estimate
{
    public partial class TransportTypeList : Form
    {
        ...
        private void TransportTypeList_Load(object sender, EventArgs e)
        {
        ...
        private void LoadTransportgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_TransportTypeGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM TransportType WHERE ID = @ID"; // Change table name properly
        ...
        {
            TransportType newForm = new TransportType(); // Create an instance of Form2
            newForm.ShowDialog();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (columnName == "Button1")
                {
                    ...
                    TransportType newForm = new TransportType();

                    newForm.Controls["textBox3"].Text = selectedRow.Cells["ID"].Value?.ToString();
                    newForm.Controls["textBox1"].Text = selectedRow.Cells["TransportName"].Value?.ToString();
                    newForm.Controls["textBox2"].Text = selectedRow.Cells["VehicleNo"].Value?.ToString();

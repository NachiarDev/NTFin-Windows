...
namespace Estimate
{
    public partial class QuotationList : Form
    {
        ...
        private void QuotationList_Load(object sender, EventArgs e)
        {
            ...
            dataGridView1.Columns["RefNo"].HeaderText = "Quotation Number";
            AddButtonColumns();
        ...
        private void LoadQuotationgrid()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("Usp_QuotationGetGrid", conn))
                    {
        ...
        private void DeleteFromDatabase(string id)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM Quotation WHERE ID = @ID"; // Change table name properly
